...
impl CPU {
    ...
    // STA (store register A to memory)
    ...
    pub fn run(&mut self) {
        loop {
            ...
            match opcode {
                ...
                /* LDA */
                // LDA with immediate addressing mode (load to register A)
                0xA9 => {
                    self.lda(&AddressingMode::Immediate);
                    self.program_counter += 1;
                }

                // LDA with zero page addressing mode (load to register A)
                0xA5 => {
                    self.lda(&AddressingMode::ZeroPage);
                    self.program_counter += 1;
                }

                // LDA with absoulute addressing mode (load to register A)
                0xAD => {
                ...
                /* STA */
                // STA with zero page addressing mode (store register A in memory)
                0x85 => {
                    self.sta(&AddressingMode::ZeroPage);
                    self.program_counter += 1;
                }

                // STA with zero page + X addressing mode (store register A in memory)
                0x95 => {
